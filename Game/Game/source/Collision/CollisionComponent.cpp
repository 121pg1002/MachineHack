...
namespace MachineHuck::Collision {
    ...
    bool CollisionComponent::CircleToLine(const Actor::Actor& act1, const Math::Vector4 start, const Math::Vector4 end)
    {

        Math::Vector4 X = start - act1.GetPosition();
        Math::Vector4 Y = end - start;

        double a = Y.Dot(Y);
        double b = 2.0 * X.Dot(Y);
        double c = X.Dot(X) - act1.GetR() * act1.GetR();
